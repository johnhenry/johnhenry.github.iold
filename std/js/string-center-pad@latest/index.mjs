const centerPad = (string, max=0, char=' ', grow=false, startBias=true ) =>{
...
export const join = (iterator, joiner=',', char=' ', grow=false, startBias=true )=>{
  const items = [...iterator];
  const max = Math.max(...items.map(x=>x.length));
  return items.map(item => centerPad(item, max, char, grow, startBias)).join(joiner);
};