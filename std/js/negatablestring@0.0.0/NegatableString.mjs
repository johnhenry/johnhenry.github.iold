...
export const NegatableString = class{
  ...
  consoleIterator(nColor=NCOLOR, pColor=COLOR, color=COLOR){
    const colors = [];
    const string = this.rep.map(([x, negative])=>{
  ...
  toString(nStart='', nEnd='', pStart='', pEnd=''){
    return this.rep.map(([x, negative])=>negative ? `${nStart}${x}${nEnd}`:`${pStart}${x}${pEnd}`).join('');
  }
  charAt(index){
...
export const invert = (string)=>
  new NegatableString(string.rep
      .map(([x, negative])=>[x, !negative]).reverse());
...
export const flip = (string) => new NegatableString(string.rep.map(([x, s]) => [x, !s]));
export const abs = (string) => new NegatableString(string.rep.map(([x, s]) => [x, false]));
export const nAbs = (string) => new NegatableString(string.rep.map(([x, s]) => [x, true]));
export const flushPositive = (string) => new NegatableString(string.rep.filter(([x, s]) => s));
export const flushNegative = (string) => new NegatableString(string.rep.filter(([x, s]) => !s));
