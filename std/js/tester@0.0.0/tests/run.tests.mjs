...
await tester('Test run without print', async function *(){
  ...
  for await (const output of run(async function*(){
    ...
    yield equal(output, expected[index], `should produce expected result: ${expected[index]}`);
    index++;
  }
  ...
  for await (const output of run(async function*(){
    ...
    yield ok(output instanceof TestError, `should produce an error: ${output.message}`);
  }
});