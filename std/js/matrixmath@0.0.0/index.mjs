...
export const map =(matrix, mapper)=>matrix.map((row)=>{
  return typeof row.map === 'function' 
  ? row.map(mapper) 
  : mapper(row);
});
...
export const createKroneckerProduct = (multiply=a*b)=>(alpha, beta)=>{ 
  return [...new Array(alpha.length)]
    .map((_, index)=>map(beta, (b)=>multiply(b, alpha[index])) );

}
export const collapse = (vector=[],binaryOperation=(a,b)=>a+b,defaultObject=0) => {
  if(vector.length === 0){
...
export const createMultiplyOld = (add=(a,b)=>a+b, multiply=(a,b)=>a*b)=>(alpha, beta)=>{
