...
const Template = ({ variant }) =>
  html`<menu-component>${variant}</menu-component>`;

<Meta title="Library/Menu Component/Demo" />

<Preview>
  <Story name="Primary" args={{ variant: "primary" }}>
    {({ variant }) => html`<menu-component class="menu" id="menu">
      <menu-item data-key="snake"
        >External Page
        <template>
          <iframe src="https://johnhenry.github.io" frameborder="0"></iframe>
          <button
