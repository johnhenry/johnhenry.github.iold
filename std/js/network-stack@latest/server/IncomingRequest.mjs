...
export default class {
  constructor(connection) {
    ...
    const { promise:methodReadyPromise
      , resolve:methodReadyResolve } = InvertedPromise();
    this.methodReady = methodReadyPromise;
    this.methodReadyResolve = methodReadyResolve;

    const { promise:pathReadyPromise
      , resolve:pathReadyResolve } = InvertedPromise();
    ...
    const { promise:headersReadyPromise
      , resolve:headersReadyResolve } = InvertedPromise();
    this.headersReady = headersReadyPromise;
    this.headersReadyResolve = headersReadyResolve;

    const { promise:headerIteratorReadyPromise
      , resolve:headerIteratorReadyResolve } = InvertedPromise();
  ...
  async init(connection, timeout=100, highWaterMark=1024){
    ...
    while (true) {
      ...
      const count = await Promise.race([timeoutPromise, connection.read(this.chunk)]);
      if (!count) {
        break;
      } else {
        const char = decoder.decode(this.chunk);
        if(this.stage < 3){
          if(char === ' ' || (chars[chars.length - 1] === '\r' && char === '\n') ){
            switch(this.stage){
              case 0:
    ...
    let totalRead = 0;
    const max = this.headers.has('content-length') ? Number(this.headers.get('content-length')) : undefined;
  ...
  // https://streams.spec.whatwg.org/#qs
    ...
    this._body = new ReadableStream({pull}, new ByteLengthQueuingStrategy ({ highWaterMark }));
