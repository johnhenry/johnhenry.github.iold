...
export default class {
  constructor(body=undefined,
      ...
      = {headers:{}, status:200, statusText:'', version:VERSION}, paused=false ) {
    this._body = [];
    if(body){
    ...
    const { promise:headersIteratorReadyPromise
      , resolve:headersIteratorReadyResolve } = InvertedPromise();
    ...
    const { promise:versionReadyPromise
      , resolve:versionReadyResolve } = InvertedPromise();
    ...
    const { promise:statusReadyPromise
      , resolve:statusReadyResolve } = InvertedPromise();
    ...
    const { promise:statusTextReadyPromise
      , resolve:statusTextReadyResolve } = InvertedPromise();
    ...
    const { promise:bodyIteratorReadyPromise
      , resolve:bodyIteratorReadyResolve } = InvertedPromise();
    ...
    const { promise:bodyStartedPromise
      , resolve:bodyStartedResolve } = InvertedPromise();
  ...
  sendStatusText(statusText, go=true){
    ...
    if(!this.statusTextSent){
      ...
      if(go){
        ...
        // this.statusTextSent = true;
        this.headersIteratorReadyResolve(this.headersIteratorController.iterator);
        // this.headersIteratorSent = true;
      }
    }
  }
  async sendHeaders(headers={}, go=true){//Everything that's there, and also some more maybe
  ...
  async sendBody(chunk=undefined, go=true){//Everything that's there, and also some more maybe
...
export const redirect = (url, status)=>new Response('', {headers:{'Location': url, status}});
