...
export const negate = (alpha=0)=>new ComplexNumber(-1*alpha, -1*alpha.i);
...
export const divide = (alpha=0, beta=0)=>multiply(alpha, reciprocal(beta));

export const equal = (alpha=1, beta=1)=>(alpha.r === beta.r && alpha.i === beta.i) || (alpha.m === beta.m && alpha.a === beta.a);

export const conjugate = (alpha=1)=>new ComplexNumber(alpha.r, -alpha.i);

export const round = (alpha) => new ComplexNumber(Math.round(alpha.r), Math.round(alpha.i) );

export const floor = (alpha) => new ComplexNumber(Math.floor(alpha.r), Math.floor(alpha.i) );

export const ceil = (alpha) => new ComplexNumber(Math.ceil(alpha.r), Math.ceil(alpha.i) );

export const trunc = (alpha, digits=0) => new ComplexNumber(
...
export const logarithm = (alpha, base = Math.E,selectA =0,selectB =0) =>{		
  return divide(naturalLogarithm(alpha, selectA), naturalLogarithm(base, selectB));
}
