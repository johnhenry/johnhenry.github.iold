...
const hydrate = (
  outputElement,
  inputElement = inputElementsByOutput.get(outputElement)
) => {
...
const render = (inputElement) => {
  if (!outputElementsByInput.get(inputElement)) {
    ...
    const outputParent =
      outputElementsByInput.get(inputElement.parentElement) || outputs;
...
const updateInput = (inputElement) => {
  inputElement.dataset.contentBeforeTag = `${inputElement.dataset.tag}[${
    inputElement.dataset.component_id
  }:${inputElement.dataset.id}] ${stringifyAttributes(
    JSON.parse(inputElement.dataset.attributes)
  )}`;
};
...
const createOutput = (inputElement) => {
  ...
  outputElement.appendChild(
    document.createTextNode(inputElement.dataset.contentBefore || "")
  );
  outputElement.appendChild(
    document.createTextNode(inputElement.dataset.contentAfter || "")
  );
...
const handleDropOnBody = (event) => {
  ...
  switch (type) {
    case "component":
      ...
      if (
        confirm(`${target.archived ? "Unarchive" : "Archive"} Component ${id}?`)
      ) {
...
const prepareTextForDownload = (text) =>
...
const setHTMLCode = () => {
  ...
  downloadBackupButton.setAttribute(
    "href",
    prepareTextForDownload(BACKUP_TEXT)
  );
...
export default async ({ path }) => {
  ...
  createComponentDefinition = window.document.getElementById(
    "create-component-definition"
  );
  createComponentContentBefore = window.document.getElementById(
    "create-component-content-before"
  );
  createComponentContentAfter = window.document.getElementById(
    "create-component-content-after"
  );
  createComponentAttributes = window.document.getElementById(
    "create-component-attributes"
  );
  createAttributeButton = window.document.getElementById(
    "create-attribute-button"
  );
  createAttributeButton.addEventListener(
    "click",
    addAttribute(createComponentAttributes)
  );
  createComponentButton = window.document.getElementById(
  ...
  updateComponentContentBefore = window.document.getElementById(
    "update-component-content-before"
  );
  updateComponentContentAfter = window.document.getElementById(
    "update-component-content-after"
  );
  updateComponentAttributes = window.document.getElementById(
    "update-component-attributes"
  );
  updateAttributeButton = window.document.getElementById(
    "update-attribute-button"
  );
  updateAttributeButton.addEventListener(
    "click",
    addAttribute(updateComponentAttributes)
  );

  updateComponentButton = window.document.getElementById(
  ...
  updateComponentAttributes.addEventListener(
    "click",
    handleComponentListDblClick
  );
  ...
  observer.observe(componentList, {
    attributes: true,
    childList: true,
    subtree: true,
  });
